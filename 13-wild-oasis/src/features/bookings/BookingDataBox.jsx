...
function BookingDataBox({ booking }) {
    const {
        ...
        observations,
        paid,
        guests: { name: guestName, email, country, country_flag, national_id },
    ...
    return (
        <StyledBookingDataBox>
            <Header>
                ...
                <p>
                    {format(new Date(start_date), "EEE, MMM dd yyyy")} ({isToday(new Date(start_date)) ? "Today" : formatDistanceFromNow(start_date)}) &mdash; {format(new Date(end_date), "EEE, MMM dd yyyy")}
                </p>
            </Header>
            ...
            <Section>
                <Guest>
                    ...
                    <p>
                        {guestName} {num_of_guests > 1 ? `+ ${num_of_guests - 1} guests` : ""}
                    </p>
                    <span>&bull;</span>
                    <p>{email}</p>
                    <span>&bull;</span>
                ...
                <Price isPaid={paid}>
                    <DataItem icon={<HiOutlineCurrencyDollar />} label={`Total price`}>
                        ...
                        {has_breakfast && ` (${formatCurrency(cabin_price)} cabin + ${formatCurrency(extras_price)} breakfast)`}
                    </DataItem>
